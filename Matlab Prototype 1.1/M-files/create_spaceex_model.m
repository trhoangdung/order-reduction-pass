...
% input : sys is an array of systems

[mS,nS] = size(sys);
for i =1:nS
    k = size(sys(i).sys.a,1); % dimension of the ith system 
    str = sprintf('_%d',k);
    Fn = [file_name str];
    Create_SpaceEx_Cfg_file(Fn,sys(i),stoptime);
    Create_SpaceEx_xml_file(Fn,sys(i));
end
end
